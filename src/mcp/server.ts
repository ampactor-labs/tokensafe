...
import { priceStringFor, mcpPaidTool } from "../discovery/catalog.js";
...
export function createMcpServer(baseUrl: string = ""): McpServer {
  const checkPrice = priceStringFor("/v1/check");
  const server = new McpServer({
    name: "tokensafe",
    version: "1.0.0",
    description:
      `Solana token safety scanner. Rug risk score 0-100 from pure on-chain analysis — mint/freeze authority, holder concentration, liquidity, LP locks, honeypot detection, Token-2022 extensions. Free lite check or ${checkPrice} USDC full report via x402.`,
  ...
  // Paid full-check tool — advertised only when enabled. Real calls are
  // intercepted by the x402 gate (src/mcp/payment.ts) before the transport;
  // this handler is the unreached fallback for non-x402 clients.
  if (config.paidMcpToolEnabled) {
    server.registerTool(
      mcpPaidTool.toolName,
      {
        title: "Solana Token Safety Check — Full Report (paid)",
        description: `${mcpPaidTool.description} Price: ${checkPrice} USDC via x402.`,
        inputSchema: {
          mint_address: z
            .string()
            .describe("Solana token mint address in base58 format"),
        ...
        content: [
          {
            ...
            text: JSON.stringify({
              error:
                "Payment required. Call this tool through an x402-enabled MCP client; the server returns an HTTP 402 payment challenge to pay and retry.",
            }),
          },
        ],
        isError: true,
