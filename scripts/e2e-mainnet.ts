...
async function main() {
  ...
  if (!SKIP_X402) {
    if (!process.env.SVM_PRIVATE_KEY) {
      console.error(
        "\x1b[31mSVM_PRIVATE_KEY required (or set SKIP_X402=1)\x1b[0m",
      );
      process.exit(1);
    }
    ...
    assert(
      ...
      `SVM_PRIVATE_KEY must decode to 64 bytes, got ${keypairBytes.length}`,
    );
  ...
  // Health
  await check(
    "GET /health — status, version, network, signer_pubkey",
    async () => {
      ...
      assert(
        body.version === "1.0.0",
        `expected version 1.0.0, got ${body.version}`,
      );
      assert(
        body.network === "mainnet",
        `expected network mainnet, got ${body.network}`,
      );
      assert(
        typeof body.signer_pubkey === "string" && body.signer_pubkey.length > 0,
        "missing signer_pubkey",
      );
      assert(
        typeof body.facilitator_url === "string",
        "missing facilitator_url",
      );
      assert(typeof body.uptime === "number", "missing uptime");
      signerPubkey = body.signer_pubkey;
      log(
        `version=${body.version} network=${body.network} uptime=${body.uptime}s`,
      );
  ...
  // Discovery doc
  await check(
    "GET /.well-known/x402 — version, resources, pricing",
    async () => {
  ...
  // Lite — wSOL (baseline)
  await check(
    "GET /v1/check/lite wSOL — LOW risk, shape validation",
    async () => {
      ...
      assert(
        body.authorities_renounced === true,
        "wSOL authorities not renounced",
      );
      assert(body.has_liquidity === true, "wSOL missing liquidity");
      assert(
        typeof body.liquidity_rating === "string",
        "missing liquidity_rating",
      );
      assert(
        body.top_10_concentration === null ||
          typeof body.top_10_concentration === "number",
        "bad top_10_concentration type",
      );
      // Data confidence fields
      assert(
        body.data_confidence === "complete" ||
          body.data_confidence === "partial",
      ...
      if (body.data_confidence === "complete") {
        assert(
          body.degraded_note === null,
          "degraded_note should be null when complete",
        );
        assert(
          body.uncertainty_penalties === null,
          "uncertainty_penalties should be null when complete",
        );
      }
      // X-Data-Confidence header
      const confidence = res.headers.get("x-data-confidence");
      assert(
        confidence === body.data_confidence,
        `X-Data-Confidence header (${confidence}) != body (${body.data_confidence})`,
      );
      ...
      log(
        `risk=${body.risk_score} (${body.risk_level}) confidence=${body.data_confidence} name=${body.name}`,
      );
    },
  );
  ...
  await check("GET /v1/check/lite USDC — LOW risk", async () => {
    ...
    assert(
      body.risk_level === "LOW",
      `USDC expected LOW, got ${body.risk_level}`,
    );
    log(
      `risk=${body.risk_score} (${body.risk_level}) is_token_2022=${body.is_token_2022}`,
    );
  });
  ...
  await check("GET /v1/check/lite TRUMP — expect elevated risk", async () => {
    ...
    log(
      `risk=${body.risk_score} (${body.risk_level}) can_sell=${body.can_sell} liq=${body.has_liquidity}`,
    );
  });
  ...
  await check("GET /v1/check/lite ai16z — log profile", async () => {
    ...
    log(
      `risk=${body.risk_score} (${body.risk_level}) can_sell=${body.can_sell} name=${body.name}`,
    );
  });
  ...
  if (WILD_MINT) {
    await check(
      `GET /v1/check/lite wild=${WILD_MINT.slice(0, 8)}...`,
      async () => {
        ...
        if (res.status === 200) {
          const body = (await res.json()) as any;
          log(
            `risk=${body.risk_score} (${body.risk_level}) name=${body.name} degraded=${body.degraded}`,
          );
        } else {
  ...
  await check("GET /v1/decide TRUMP threshold=30 → RISKY", async () => {
    const res = await fetch(
      `${BASE}/v1/decide?mint=${MINTS.TRUMP}&threshold=30`,
    );
    assert(res.status === 200, `expected 200, got ${res.status}`);
    const decideConfidence = res.headers.get("x-data-confidence");
    assert(
      decideConfidence === "complete" || decideConfidence === "partial",
      `missing/bad X-Data-Confidence on decide: ${decideConfidence}`,
    );
    const body = (await res.json()) as any;
    assert(
      body.decision === "RISKY" || body.decision === "UNKNOWN",
      `expected RISKY|UNKNOWN for TRUMP, got ${body.decision}`,
    );
    log(
      `decision=${body.decision} risk=${body.risk_score} threshold=${body.threshold_used} confidence=${decideConfidence}`,
    );
  });

  await check(
    "GET /v1/decide wSOL → SAFE or UNKNOWN (if degraded)",
    async () => {
      const res = await fetch(`${BASE}/v1/decide?mint=${MINTS.wSOL}`);
      assert(res.status === 200, `expected 200, got ${res.status}`);
      const body = (await res.json()) as any;
      assert(
        body.decision === "SAFE" || body.decision === "UNKNOWN",
        `expected SAFE|UNKNOWN for wSOL, got ${body.decision}`,
      );
  ...
  await check(
    "POST /mcp tools/list — solana_token_safety_check present",
    async () => {
      ...
      assert(
        ...
        `missing tool, got: ${names.join(", ")}`,
      );
      assert(
        !names.includes("solana_token_safety_preview"),
        "deprecated tool still present",
      );
    },
  );

  await check(
    "POST /mcp tools/call wSOL — lite data, absolute URL",
    async () => {
      const res = await fetch(`${BASE}/mcp`, {
        method: "POST",
        headers: mcpHeaders,
        body: JSON.stringify({
          ...
          params: {
            ...
            arguments: { mint_address: MINTS.wSOL },
          },
        }),
      });
      const text = await res.text();
      ...
      assert(
        ...
        `full_report.url not absolute: ${result.full_report.url}`,
      );
    },
  );
  ...
  await check("Non-mint account → 404 TOKEN_NOT_FOUND", async () => {
    const res = await fetch(
      `${BASE}/v1/check/lite?mint=TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA`,
    );
  ...
  await check("GET /metrics with bearer → 200, Prometheus data", async () => {
    ...
    const text = await res.text();
    assert(
      text.includes("tokensafe_http_requests_total"),
      "missing tokensafe_http_requests_total metric",
    );
  });
  ...
  await check("POST /v1/api-keys → 201, tks_ prefix", async () => {
    ...
    const body = (await res.json()) as any;
    assert(
      body.key.startsWith("tks_"),
      `key should start with tks_, got ${body.key.slice(0, 8)}`,
    );
  ...
  await check(
    "GET /v1/api-keys — lists keys, full key NOT exposed",
    async () => {
      const res = await fetch(`${BASE}/v1/api-keys`, {
  ...
  await check("POST /v1/webhooks → 201 with HMAC secret", async () => {
    ...
    assert(
      typeof body.secret_hmac === "string" && body.secret_hmac.length > 16,
      "missing/short secret_hmac",
    );
    webhookId = body.id;
    log(
      `id=${webhookId} threshold=${body.threshold} secret=***${body.secret_hmac.slice(-8)}`,
    );
  });
  ...
  await check("GET /v1/webhooks — secret redacted", async () => {
    ...
    assert(
      found.secret_hmac.startsWith("***"),
      `secret not redacted: ${found.secret_hmac.slice(0, 10)}`,
    );
  });
  ...
  await check("PATCH /v1/webhooks/:id — threshold updated", async () => {
    ...
    assert(
      body.threshold === 25,
      `expected threshold 25, got ${body.threshold}`,
    );
    assert(
      body.callback_url === "https://httpbin.org/post",
      "callback_url changed unexpectedly",
    );
  ...
  await check(
    "GET /v1/check wSOL via API key — 200, full response",
    async () => {
      const res = await fetch(`${BASE}/v1/check?mint=${MINTS.wSOL}`, {
      ...
      assert(
        res.headers.get("x-api-key-tier") === "pro",
        `expected tier pro, got ${res.headers.get("x-api-key-tier")}`,
      );
      assert(
        res.headers.get("x-api-key-usage") !== null,
        "missing X-API-Key-Usage",
      );
      assert(
        res.headers.get("access-control-allow-origin") === "*",
        "missing CORS on paid endpoint",
      );
      const body = (await res.json()) as any;
      assert(
        body.checks.mint_authority.status === "RENOUNCED",
        `wSOL mint_authority not RENOUNCED`,
      );
      assert(
        body.checks.freeze_authority.status === "RENOUNCED",
        `wSOL freeze_authority not RENOUNCED`,
      );
      assert(
        body.checks.liquidity?.has_liquidity === true,
        "wSOL no liquidity",
      );
      assert(
        typeof body.response_signature === "string",
        "missing response_signature",
      );
      assert(
        typeof body.score_breakdown === "object",
        "missing score_breakdown",
      );
      assert(
        body.signer_pubkey === signerPubkey!,
        `signer_pubkey mismatch: health=${signerPubkey!.slice(0, 8)} check=${body.signer_pubkey?.slice(0, 8)}`,
      );
      // Data confidence
      assert(
        body.data_confidence === "complete" ||
      ...
      assert(
        checkConfidence === body.data_confidence,
        `X-Data-Confidence header mismatch`,
      );
      ...
      if (liq?.primary_pool?.toLowerCase().includes("raydium")) {
        log(
          `lp_locked=${liq.lp_locked} lp_mint=${liq.lp_mint?.slice(0, 12)}... lp_locker=${liq.lp_locker}`,
        );
        // pool_vault_addresses should be present for Raydium AMM v4 pools
        if (liq.pool_vault_addresses) {
          assert(
            Array.isArray(liq.pool_vault_addresses),
            "pool_vault_addresses should be array",
          );
          assert(
            liq.pool_vault_addresses.length === 2,
            `expected 2 vault addresses, got ${liq.pool_vault_addresses.length}`,
          );
          log(
            `pool_vault_addresses: [${liq.pool_vault_addresses.map((a: string) => a.slice(0, 8) + "...").join(", ")}]`,
          );
        } else {
          log(
            `pool_vault_addresses: null (non-AMM-v4 pool or LP lock skipped)`,
          );
        }
      }
      log(
        `risk=${body.risk_score} (${body.risk_level}) mint_auth=${body.checks.mint_authority.status} freeze_auth=${body.checks.freeze_authority.status}`,
      );
  ...
  await check(
    "GET /v1/check TRUMP via API key — elevated risk or degraded",
    async () => {
      const res = await fetch(`${BASE}/v1/check?mint=${MINTS.TRUMP}`, {
        headers: apiKeyHeaders,
      });
      assert(res.status === 200, `expected 200, got ${res.status}`);
      const body = (await res.json()) as any;
      ...
      if (body.degraded) {
        assert(
          body.risk_score >= 5,
          `degraded TRUMP risk too low: ${body.risk_score}`,
        );
        log(
          `DEGRADED — risk=${body.risk_score} (${body.risk_level}) degraded_checks=${JSON.stringify(body.degraded_checks)}`,
        );
      } else {
        assert(
          body.risk_score > 15,
          `non-degraded TRUMP risk too low: ${body.risk_score}`,
        );
        log(`risk=${body.risk_score} (${body.risk_level})`);
      }
      log(
        `mint_auth=${body.checks.mint_authority?.status} freeze_auth=${body.checks.freeze_authority?.status}`,
      );
      const th = body.checks.top_holders;
      log(
        `top10=${th?.top_10_percentage}% top1=${th?.top_1_percentage}% status=${th?.status}`,
      );
      if (th?.note) log(`top_holders note: ${th.note}`);
      const tliq = body.checks.liquidity;
      log(
        `liquidity=${tliq?.status} (${tliq?.liquidity_rating}) vaults=${JSON.stringify(tliq?.pool_vault_addresses?.map((a: string) => a.slice(0, 8)) ?? null)}`,
      );
  ...
  await check("POST /v1/check/batch/medium — 5 memecoins", async () => {
    const res = await fetch(`${BASE}/v1/check/batch/medium`, {
      ...
      body: JSON.stringify({
        mints: [MINTS.TRUMP, MINTS.ai16z, MINTS.WIF, MINTS.BONK, MINTS.POPCAT],
      }),
    });
    assert(res.status === 200, `expected 200, got ${res.status}`);
    const body = (await res.json()) as any;
    ...
    for (const r of body.results) {
      if (r.risk_score !== undefined) {
        log(
          `  ${(r.symbol ?? r.mint.slice(0, 8)).padEnd(8)} risk=${String(r.risk_score).padStart(3)} (${r.risk_level}) degraded=${r.degraded ?? false}`,
        );
      } else {
  ...
  await check(
    "POST /v1/check/batch/large — 10 tokens (full roster)",
    async () => {
      ...
      const body = (await res.json()) as any;
      assert(
        body.total === allMints.length,
        `expected total=${allMints.length}, got ${body.total}`,
      );
      log(
        `total=${body.total} succeeded=${body.succeeded} failed=${body.failed}`,
      );
      ...
      for (const r of sorted) {
        log(
          `    ${(r.symbol ?? r.mint.slice(0, 8)).padEnd(10)} ${String(r.risk_score).padStart(4)} ${r.risk_level}`,
        );
      }
      ...
      if (wsolResult?.risk_score !== undefined) {
        assert(
          wsolResult.risk_score <= 20,
          `wSOL risk too high in batch: ${wsolResult.risk_score}`,
        );
      }
      if (usdcResult?.risk_score !== undefined) {
        assert(
          usdcResult.risk_score <= 20,
          `USDC risk too high in batch: ${usdcResult.risk_score}`,
        );
      }
      if (
        trumpResult?.risk_score !== undefined &&
        wsolResult?.risk_score !== undefined
      ) {
        // TRUMP may return degraded (top_holders timeout) — only compare when non-degraded
        if (!trumpResult.degraded) {
          assert(
            trumpResult.risk_score > wsolResult.risk_score,
            `TRUMP (${trumpResult.risk_score}) should score higher than wSOL (${wsolResult.risk_score})`,
          );
        } else {
  ...
  await check("POST /v1/audit/small — TRUMP, WIF, BONK", async () => {
    ...
    assert(
      body.attestation?.signer_pubkey === signerPubkey!,
      "signer_pubkey mismatch in audit",
    );
    ...
    const expectedHash = crypto
      ...
      .update(
        JSON.stringify({
          mints: [MINTS.TRUMP, MINTS.WIF, MINTS.BONK],
      ...
      .digest("hex");
    assert(
      body.attestation.hash === expectedHash,
      `attestation hash mismatch: expected ${expectedHash.slice(0, 16)}... got ${body.attestation.hash.slice(0, 16)}...`,
    );

    log(`audit_id=${auditId}`);
    log(
      `aggregate_risk=${body.aggregate_risk_score} violations=${body.policy_violations?.length ?? 0}`,
    );
  ...
  await check("POST /v1/audit/standard — full 10-token roster", async () => {
    ...
    log(
      `audit_id=${auditStandardId} aggregate_risk=${body.aggregate_risk_score} violations=${body.policy_violations?.length ?? 0}`,
    );
  ...
  if (SKIP_X402) {
    ...
    await check("GET /v1/check wSOL via x402 ($0.008)", async () => {
      ...
      log(
        `risk=${body.risk_score} (${body.risk_level}) receipt=${receipt!.slice(0, 40)}...`,
      );
    });
    ...
    await check("POST /v1/check/batch/small via x402 ($0.025)", async () => {
      ...
      assert(
        body.succeeded >= 2,
        `expected ≥2 succeeded, got ${body.succeeded}`,
      );
      log(
        `total=${body.total} succeeded=${body.succeeded} receipt=${receipt!.slice(0, 40)}...`,
      );
    });
  }
  ...
  await check("GET /v1/audit/history (bearer) — contains audit", async () => {
    const res = await fetch(`${BASE}/v1/audit/history`, {
      headers: bearerHeaders,
    });
    assert(res.status === 200, `expected 200, got ${res.status}`);
    const body = (await res.json()) as any;
  ...
  await check("GET /v1/audit/history?mint=wSOL — filtered", async () => {
    const res = await fetch(`${BASE}/v1/audit/history?mint=${MINTS.wSOL}`, {
      headers: bearerHeaders,
    });
    assert(res.status === 200, `expected 200, got ${res.status}`);
    const body = (await res.json()) as any;
  ...
  await check("GET /v1/audit/:id/report (bearer) — markdown", async () => {
    const res = await fetch(`${BASE}/v1/audit/${auditId}/report`, {
      headers: bearerHeaders,
    });
    assert(res.status === 200, `expected 200, got ${res.status}`);
  ...
  await check("GET /v1/audit/fake-uuid/report → 404", async () => {
    const res = await fetch(
      `${BASE}/v1/audit/00000000-0000-0000-0000-000000000000/report`,
      { headers: bearerHeaders },
    );
    assert(res.status === 404, `expected 404, got ${res.status}`);
  });
  ...
  await check("GET /v1/api-keys/:id/usage — used > 0", async () => {
    const res = await fetch(`${BASE}/v1/api-keys/${apiKeyId}/usage`, {
      headers: bearerHeaders,
    });
    assert(res.status === 200, `expected 200, got ${res.status}`);
    const body = (await res.json()) as any;
  ...
  await check("Revoked key rejected", async () => {
    ...
    assert(
      res.status === 401,
      `expected 401 for revoked key, got ${res.status}`,
    );
  });
  ...
  for (const p of phases) {
    ...
    console.log(
      `  ${status} ${p.name.padEnd(38)} ${String(p.passed).padStart(4)}  ${String(p.failed).padStart(4)}  ${String(p.skipped).padStart(4)}`,
    );
  }
  console.log(`  ${"─".repeat(55)}`);
  console.log(
    `  ${"Total".padEnd(40)} ${String(totalPassed).padStart(4)}  ${String(totalFailed).padStart(4)}  ${String(totalSkipped).padStart(4)}`,
  );
  console.log();
