...
const CATEGORIES: Record<
  string,
  { label: string; expectMin: number; expectMax: number }
> = {
  trusted: { label: "Trusted Authority Tokens", expectMin: 0, expectMax: 50 },
  untrusted: {
    label: "Active Untrusted Authority",
    expectMin: 5,
    expectMax: 50,
  },
...
const STATIC_TOKENS: TokenEntry[] = [
  ...
  },
  {
    ...
    expectMax: 50,
    fullCheck: false,
    note: "LST, trusted mint auth, high holder concentration",
  },
  {
    symbol: "PYUSD",
    ...
    expectMax: 50,
    fullCheck: true,
    note: "Token-2022, Paxos, high holder concentration",
  },
  ...
  // Category 2: Active Untrusted Authority
  {
    ...
    expectMax: 50,
    fullCheck: true,
    note: "NOT trusted, deep liquidity + established",
  },
  {
    symbol: "INF",
    ...
    expectMax: 50,
    fullCheck: false,
    note: "Sanctum multi-LST, not trusted",
  },
  ...
  // Category 3: High Concentration
  {
    ...
    expectMax: 70,
    fullCheck: true,
    note: "top10=91.5%, top1=76.8%",
  },

  // Category 4: Rugged / Zero Liquidity
  {
    symbol: "SHAR",
    mint: "DCfVHxrnLfHQuyjiMdHdnbn3NAAnPX8Ny15WNgofpump",
    category: "rugged",
    expectMin: 50,
    expectMax: 100,
    fullCheck: true,
    note: "Confirmed rug, $0 liquidity, pump.fun Token-2022",
  },

  // Category 5: Token-2022 Extensions
  {
    symbol: "PD-test",
    mint: "aNMXxywEHAH3VfWnaVwedLJWPT9NsxagsGTEQqS5WKK",
    category: "extensions",
    expectMin: 20,
    expectMax: 100,
    fullCheck: true,
    note: "PermanentDelegate example",
  },

  // Category 6: Fresh pump.fun — 1 known + 3 dynamic
  {
    symbol: "MeuW...",
    mint: "MeuWvMQTbze3BMzhKuD9TQhLmVwtwUqA6PFPPV1pump",
    category: "fresh",
    expectMin: 25,
    expectMax: 100,
    fullCheck: true,
    note: "Known pump.fun, score=58 last session",
  },
  ...
  },
  {
    ...
    expectMax: 40,
    fullCheck: false,
    note: "DeFi blue chip, may have holder concentration",
  },
];
...
async function fetchLite(mint: string): Promise<LiteResult> {
  ...
  if (!confidence || (confidence !== "complete" && confidence !== "partial")) {
    throw new Error(
      `missing or invalid X-Data-Confidence header: ${confidence}`,
    );
  }
  return res.json();
}
...
async function fetchFreshPumpTokens(count: number): Promise<TokenEntry[]> {
  try {
    ...
    console.log(
      `  \x1b[33m⚠\x1b[0m Could not fetch DexScreener profiles: ${(err as Error).message}`,
    );
    return [];
  }
}
...
function printRow(row: AuditRow) {
  ...
  if (l) {
    ...
    if (l.data_confidence === "partial")
      details += `  ${YELLOW}partial=${JSON.stringify(l.degraded_checks)}${RESET}`;
  } else if (row.error) {
    details = `${RED}${row.error}${RESET}`;
  }

  console.log(
    `  ${verdictIcon(row.verdict)} ${sym} ${color}${score} ${level}${RESET}  ${details}`,
  );
}
...
function printFullDetails(row: AuditRow) {
  ...
  if (checks.top_holders) {
    const th = checks.top_holders as {
    ...
    if (th.status === "OK") {
      console.log(
        `    ${DIM}holders: top10=${th.top_10_percentage}%, top1=${th.top_1_percentage}%${RESET}`,
      );
    } else {
  ...
  if (checks.liquidity) {
    const lq = checks.liquidity as {
      liquidity_rating?: string;
      lp_locked?: boolean | null;
      pool_address?: string;
    };
    console.log(
      `    ${DIM}liquidity: rating=${lq.liquidity_rating}, lp_locked=${lq.lp_locked}, pool=${(lq.pool_address ?? "none").slice(0, 12)}...${RESET}`,
    );
  }
  if (checks.token_2022_extensions) {
    ...
    if (Array.isArray(ext) && ext.length > 0) {
      console.log(
        `    ${DIM}extensions: ${ext.map((e) => e.name).join(", ")}${RESET}`,
      );
    }
  }
}
...
async function main() {
  ...
  console.log(
    `${BOLD}  TOKENSAFE ACCURACY AUDIT — ${new Date().toISOString().split("T")[0]}${RESET}`,
  );
  console.log(`${BOLD}  Target: ${BASE}${RESET}`);
  console.log(
    `${BOLD}  API Key: ${API_KEY ? API_KEY.slice(0, 8) + "..." : "(none — full checks will use x402 gate)"}${RESET}`,
  );
  ...
  try {
    ...
    console.log(
      `  ${GREEN}✓${RESET} Server reachable, network=${body.network}\n`,
    );
  } catch {
  ...
  console.log(
    `${DIM}Fetching fresh pump.fun tokens from DexScreener...${RESET}`,
  );
  ...
  console.log(
    `${BOLD}Phase 1: Lite checks (${allTokens.length} tokens, ~${Math.ceil(allTokens.length * 2.5)}s)${RESET}\n`,
  );
  ...
  for (let i = 0; i < allTokens.length; i++) {
    ...
    try {
      ...
      console.log(
        `  ${icon} ${pad(lite.symbol ?? entry.symbol, 12)} ${color}${String(lite.risk_score).padStart(3)} ${lite.risk_level}${RESET}`,
      );
    } catch (err) {
      ...
      console.log(
        `  ${YELLOW}⚠${RESET} ${pad(entry.symbol, 12)} ${RED}ERROR: ${msg.slice(0, 60)}${RESET}`,
      );
    }
  ...
  const fullTargets = results.filter(
    (r) => r.entry.fullCheck && r.lite !== null,
  );
  ...
  if (fullTargets.length > 0 && API_KEY) {
    ...
    for (const row of fullTargets) {
      try {
        ...
        row.full = full;
        console.log(
          `  ${GREEN}✓${RESET} ${pad(row.lite!.symbol ?? row.entry.symbol, 12)} full check complete`,
        );
      } catch (err) {
        console.log(
          `  ${YELLOW}⚠${RESET} ${pad(row.entry.symbol, 12)} full check failed: ${(err as Error).message.slice(0, 60)}`,
        );
      }
      await sleep(2000);
    }
  ...
  for (const catKey of categoryOrder) {
    ...
    console.log(
      `${CYAN}${cat.label}${RESET} ${DIM}(expect: ${cat.expectMin}-${cat.expectMax})${RESET}`,
    );
  ...
  console.log(
    `  ${pad("Category", 35)} ${pad("Tokens", 7)} ${pad("Pass", 5)} ${pad("Warn", 5)} ${pad("Fail", 5)}`,
  );
  ...
  for (const catKey of categoryOrder) {
    ...
    console.log(
      `  ${pad(cat.label, 35)} ${pad(String(rows.length), 7)} ${pad(String(pass), 5)} ${pad(warnStr, warn > 0 ? 14 : 5)} ${pad(failStr, fail > 0 ? 14 : 5)}`,
    );
  }
  ...
  console.log(
    `  ${pad("Total", 35)} ${pad(String(total), 7)} ${pad(String(totalPass), 5)} ${pad(totalWarnStr, totalWarn > 0 ? 14 : 5)} ${pad(totalFailStr, totalFail > 0 ? 14 : 5)}`,
  );
  ...
  if (surprises.length > 0) {
    ...
    for (const s of surprises) {
      ...
      const icon =
        s.verdict === "FAIL" ? `${RED}FAIL${RESET}` : `${YELLOW}WARN${RESET}`;
      console.log(
        `  ${icon} ${s.lite?.symbol ?? s.entry.symbol} (${s.entry.category}): score=${score}, expected=${expected}, note="${s.entry.note}"`,
      );
      if (s.full) {
